...
(DEFUN ISOPRP (X) (IF (SYMBOLP X)
                      (GET (INTERN (STRING-UPCASE X)
                                  "AI.LANG.LISP.CODE.ITER.FOR.INTERNAL")
                           'FORWORD)))
...
#|(DEFMACRO SELECTC (SELECTOR &REST CASES)
   `(CASE ,SELECTOR ,@(.FOR CASE ON CASES WHILE (CDR CASE)
                                     COLLECT
                                     (CONS (EVAL (CAAR CASE))
                                           (CDAR CASE))) (OTHERWISE ,(CAR (LAST CASES)))))|#
            ...
            (OR (EQL IV (CADR (CADDAR UPDATES)))
                (PUSH `(LIST 'INCF ',IV (IF (CONSTANTP ,INCREM)
                                            ...
                                            (LIST 'SETQ ,(INTERN (CONCATENATE 'STRING "$$INCVAR"
                                                                ...
                                                                "AI.LANG.LISP.CODE.ITER.FOR.INTERNAL") ,INCREM))) UPDATES))))
(SETF (GET 'IN 'FORWORD)
      '(PROG (INCEXPR)
             (SETQ IOV (GENSYM))
...
#|(SETF (GET 'IL:FORMACRO 'IL:MAKEFILE-ENVIRONMENT)
      '(:PACKAGE "AI.LANG.LISP.CODE.ITER.FOR.INTERNAL" :READTABLE "XCL"))|#
